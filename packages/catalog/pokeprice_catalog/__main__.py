"""CLI: ``python -m pokeprice_catalog <command> [options]``.
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser("pokeprice_catalog")
...
async def _dispatch(args: argparse.Namespace) -> int:
    if args.command == "seed-sets":
        from pokeprice_catalog import seed_sets

        await seed_sets.run()
        return 0
    if args.command == "bootstrap-set":
        from pokeprice_catalog import bootstrap

        await bootstrap.run(args.sets, no_fetch=args.no_fetch)
        return 0
    if args.command == "seed-cards":
        from pokeprice_catalog import seed_cards

        await seed_cards.run(args.sets)
        return 0
    if args.command == "verify":
        from pokeprice_catalog import verify
